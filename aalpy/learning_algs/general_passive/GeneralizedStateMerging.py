    ...
    def run(self, data: Any, convert: bool = True, instrumentation: Instrumentation | None = None,
        ...
        while len(blue_states) != 0:
            ...
            if isinstance(best_candidate, GsmNode):
                ...
                # promoting a state affects the cached partitioning information (what states are freshly blue) -> invalidate
                for key in [key for key, p in partition_candidates.items() if best_candidate in p.full_mapping]:
                    del partition_candidates[key]
                # TODO: update merge candidates instead of invalidating the cache
                # # update cached partitions
                # for partitioning in partition_candidates.values():
                #     updated_promoted_node = partitioning.red_mapping.get(best_candidate)
                ...
                #         if trans is None or (out_sym not in trans and unknown_output not in trans):
